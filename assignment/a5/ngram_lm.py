...
class AddKTrigramLM(BaseLM):
    ...
    def next_word_proba(self, word, seq):
        ...
        temp_C_abc = self.counts.get(context)
        if temp_C_abc is None:
            C_abc = 0
        else:
            C_abc = self.counts.get(context).get(word)         
        
        C_ab = self.context_totals.get(context)
        ...
        if C_ab is None:
            ...
            my_denom = C_ab + (k*self.V)
        

        result = my_num/my_denom
        return result
